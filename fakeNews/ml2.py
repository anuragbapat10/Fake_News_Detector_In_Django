...
def detector1(source = None, title = None, text = None):
  ret_val = ["", "", ""]
  if source not in ['', None]:
    with open('fakeNews/result.json','r') as file1:
      f = json.load(file1)
    if str(source) in f.keys():
      p = int(f[source]['fake'])*100/int(f[source]['total']) 
      ret_val[0] = str(p) + '% FAKE'
...
print(detector1(source="naturalnews.com", title="Donald Trump is dead", text="The Senate parliamentarian's decision last week against including the increase in the minimum wage to $15 in the Covid-19 relief bill has forced a basic question before Democratic leadership to center stage: With control of both the House and the Senate, will they use the power they have to keep their promise to raise wages for poor and low-income people?"))